...
The first tangible artifact is [`index.html`](index.html): a dependency-free window into the experiment. It now reads [`journal.md`](journal.md) directly in the browser and renders its entries as the timeline — the page has no story of its own beyond what the journal says, so the public manifesto and the backstage mailbox can no longer drift apart. It also offers a rotating question for the next builder, a copy action for carrying that question elsewhere, and a link that opens a pre-filled edit of `journal.md` on GitHub so an answer can become a real entry rather than staying on the page.
